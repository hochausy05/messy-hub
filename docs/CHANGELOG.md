...
- **Phạm vi và phát hiện:** Rà soát cô lập mã theo route, ranh giới client, vòng đời GSAP/R3F, tài nguyên, asset, prefetch, giảm chuyển động và fallback WebGL trên toàn bộ route MVP. Kinetic Focus Grid, Spatial Node Field và các vòng đời GSAP hiện có đã đúng; hai vấn đề thực tế là Canvas Home vẫn render liên tục khi giảm chuyển động hoặc ngoài vùng nhìn thấy, và `/links` vẫn tải GSAP dù danh sách liên kết đang rỗng.
- **Sửa lỗi:** Chuyển Canvas Home sang render theo nhu cầu, dừng vòng lặp khi giảm chuyển động, tab ẩn hoặc Canvas ngoài màn hình; giải phóng context thăm dò WebGL2 và thay Canvas bằng fallback cục bộ khi mất context. Thêm `DestinationCardEntry` tải động để GSAP của thẻ chỉ tải khi có thẻ thật.
- **Kết quả cô lập và fallback:** `/lab` và `/profile` chỉ dùng mã shell; Home Three.js vẫn tải động và tách khỏi route khác; Kinetic và Spatial có chunk độc lập, route này không tải implementation của route kia; `/links` rỗng không còn tải runtime GSAP. DOM, điều hướng và kích thước vùng dự phòng vẫn độc lập với WebGL; không thêm asset, dependency hay tính năng hình ảnh.
- **Kiểm tra:** Bản dựng production sạch và toàn bộ sáu route trả HTTP 200; chuỗi chuyển route yêu cầu hoàn tất, Home và Spatial render sau biên tải động, không ghi nhận lỗi ứng dụng/hydration hay tích lũy Canvas. `npm run build`, `npm run lint -- src` và `git diff --check` đều đạt; đường lỗi phần cứng/context được kiểm tra theo cấu trúc fallback cục bộ vì công cụ không giả lập tin cậy hỏng GPU vật lý.
- **Tệp:** `src/components/sections/links/links-directory.tsx`, `src/components/ui/destination-card-entry.tsx`, `src/components/three/canvas/home-canvas-boundary.tsx`, `src/components/three/canvas/home-canvas.tsx`, `src/components/three/objects/hub-system.tsx`, `TASKS.md`, `docs/CHANGELOG.md`.

## 2026-09-20 — TASK-021

- **Phạm vi:** Hoàn tất rà soát sẵn sàng phát hành cho MVP, gồm bản dựng production, sáu route hiện hành, route Lab không tồn tại, luồng điều hướng, nội dung đã phê duyệt, metadata, dependency, fallback, responsive và cô lập mã tăng cường.
- **Phát hiện và sửa lỗi:** Không có lỗi chức năng hay runtime chặn phát hành. Đã loại bỏ favicon tam giác Vercel được Next.js phục vụ tự động cùng năm SVG mẫu không được tham chiếu của dự án khởi tạo; không tạo biểu tượng thương hiệu thay thế và không thay đổi giao diện hoặc tính năng.
...
- **Phạm vi & Thay đổi:** Hoàn thành tài liệu kiến trúc kỹ thuật thẩm quyền cho World Journey tại `docs/architecture/WORLD_JOURNEY_ARCHITECTURE.md`. Chốt 12 quyết định kiến trúc (WJA-001 đến WJA-012) bao gồm: sử dụng một Canvas R3F duy nhất ở cấp route cho toàn bộ hành trình thay vì sáu Canvas độc lập; cuộn dọc native là đầu vào duy nhất của người dùng; một ScrollTrigger GSAP duy nhất cung cấp `globalProgress` (0→1) cho Three.js qua ref; camera liên tục di chuyển trên đường cong CatmullRomCurve3 xuyên suốt sáu chapter; chuyển tiếp Cave → Ocean ngang được mã hóa trong hình học spline mà không cần cuộn ngang; chapter là module scene cô lập chia sẻ Canvas/camera/render pipeline; tài nguyên nặng tải lũy tiến theo vị trí cuộn; DOM chapter độc lập khỏi WebGL; mobile giảm độ phức tạp nhưng giữ nguyên cấu trúc; giảm chuyển động giảm cả khối lượng tính toán thực; greybox (TASK-024) xác nhận kiến trúc trước khi dựng asset; không thêm thư viện quản lý trạng thái toàn cục.
- **Nội dung kiến trúc:** Mô hình tiến trình chapter chuẩn hóa (global 0→1, chapter-local 0→1) với cấu hình tập trung; phân quyền DOM/Canvas rõ ràng; chiến lược kích hoạt chapter (inactive → preloading → nearby → active); chiến lược render vòng lặp hybrid; quản lý trạng thái bằng ref thay vì React state; phân chia trách nhiệm GSAP/Three.js qua hợp đồng progress duy nhất; chiến lược responsive ba tầng (desktop/tablet/mobile); kiến trúc giảm chuyển động; hợp đồng fallback không-WebGL; ngân sách hiệu năng kiến trúc; cơ chế đình chỉ khi ẩn/rời route; cô lập lỗi; hỗ trợ debug/greybox; và bàn giao cụ thể cho TASK-024.
- **Mã nguồn & Phụ thuộc:** Không thay đổi mã nguồn sản phẩm, không thay đổi giao diện ứng dụng, không bổ sung dependency.
- **Tệp:** `docs/architecture/WORLD_JOURNEY_ARCHITECTURE.md`, `TASKS.md`, `docs/CHANGELOG.md`.
